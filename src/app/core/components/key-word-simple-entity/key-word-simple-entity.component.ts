...
@Component({
  selector: 'app-key-word-simple-entity',
  templateUrl: './key-word-simple-entity.component.html',
  styleUrls: ['./key-word-simple-entity.component.scss']
})
export class KeyWordSimpleEntityComponent implements OnInit, OnChanges, OnDestroy {
  @Input() document: any;
  @Input() entities: Entity[] = [];
  @Input() values: any[] = [];
  @Input() loadKeywordsSimples: boolean = false;
  ...
  ngOnChanges(simpleChanges: SimpleChanges): void {
    if(simpleChanges?.loadKeywordsSimples?.currentValue === true){
      this.loadEntitiesPersistenses()
    }
    this.getInitialData();
  }
  ...
  public filterListDoctype(filter: any, entity: any) {
    let attributes: any = ''
    for (const _entity in this.fields) {
      if(_entity === entity.id){
        attributes = this.fields[_entity];
        break
      }
    }

    attributes.forEach( (attr: any) => {
      if(attr.templateOptions.label.toUpperCase().includes(filter.value.toUpperCase())) {
        attr.isShow = true;
      }else{
        attr.isShow = false;
      }
