...
import {
  ...
  styleTableUser
} from "@app/modules/administration/modules/teacher/models/model-teacher/constans-teacher";
import {Response, Roles, User} from "@app/core/models";
import {Router} from "@angular/router";
import {DataContentUser} from "@app/modules/administration/modules/teacher/models/model-teacher/model-teacher";
...
export class TeacherComponent implements OnInit {
  ...
  public getTeacher(): void {
    ...
    this._subscription.add(
      this.Teacherervice.getStudentsAll().subscribe((res: any) => {
        if (!res.error) {
  ...
  public lockUnlock(user: User) {
    if (user.status !== 16) {
      this.Teacherervice.blockUser('').subscribe((res: Response) => {
        if (res.error) {
          this._messageService.add( {
              type: 'error',
              message: res.msg,
      ...
      this.Teacherervice.unblockUser('').subscribe((res: Response) => {
        if (res.error) {
          this._messageService.add( {
              type: 'error',
              message: res.msg,
  ...
  public async createUserOfFile(data: any[]) {
    ...
    if (data.length) {
      ...
      await Promise.all(ArrayUser).then((Teacher: User[]) => {
        Teacher.forEach((user: User) => {
          this._subscription.add(
            this.Teacherervice.createUser(user).subscribe((resp: any) => {
              if (resp.error) {
                this.showLoader[0].value = false;
                this._messageService.add({
                  type: 'error',
  ...
  public getCreateAtMax() {
    // @ts-ignore
    this.dateMax = new Date(Math.max(...this.Teacher.map((user: User) => new Date(user.created_at))));
  }
  ...
  confirmDialogDelete(event: boolean) {
    ...
    const user = this.Teacherelected;
    if (event) {
      this.showLoader[0].value = true;
      this.Teacherervice.deleteUser(user).subscribe((res: any) => {
        if (res.error) {
          this.showLoader[0].value = false;
          this._messageService.add( {
              type: 'error',
