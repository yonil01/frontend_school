import { Component, OnInit } from '@angular/core';
import {TableModel} from "@app/ui/components/table/model/table.model";
import {
  dataContent,
  showLoader, showToast,
  styleTablePayment
} from "@app/modules/administration/modules/payments/models/model-Payment/constans-Payment";
import {Response, Roles, Payment} from "@app/core/models";
import {Router} from "@angular/router";
import {PaymentsService} from "@app/modules/administration/modules/payments/service/payment/payments.service";
import {DataContentPayment} from "@app/modules/administration/modules/payments/models/model-payment/model-payment";
import {ToastService} from "ecapture-ng-ui";
import {ConfirmationService, Message, MessageService} from "primeng/api";
import {ToastStyleModel} from "ecapture-ng-ui/lib/modules/toast/model/toast.model";
import {toastDataStyle} from "@app/core/models/toast/toast";
import * as XLSX from 'xlsx';
type AOA = any[][];
// @ts-ignore
import { v4 as uuidv4 } from 'uuid';
import {encryptText} from "@app/core/utils/crypto/cypher";
import {Store} from "@ngrx/store";
import {AppState} from "@app/core/store/app.reducers";
import {Subscription} from "rxjs/internal/Subscription";

@Component({
  selector: 'app-Payments',
  templateUrl: './payments.component.html',
  styleUrls: ['./payments.component.scss'],
  providers: [ConfirmationService, MessageService],
})
export class PaymentsComponent implements OnInit {
  public showPassword: boolean = false;
  public styleTable: TableModel = styleTablePayment;
  public Payments: Payment[] = [];
  public Payment: Payment;
  public PaymentSelected: Payment = {};
  public contentModel: DataContentPayment[] = dataContent
  public showEdit: boolean;
  public showLoader: any = showLoader;
  public toastStyle: ToastStyleModel = toastDataStyle;
  public dateMax = new Date();
  private _subscription: Subscription = new Subscription();
  public secretKey: string;
  public dataExport:any =[];
  msgs: Message[] = []
  data: AOA = [[1, 2], [3, 4]];
  wopts: XLSX.WritingOptions = { bookType: 'xlsx', type: 'array' };
  fileName: string = 'Usuarios No creado.xlsx';
  public showConfirmDelete: boolean = false;

  constructor(private router: Router,
              private PaymentService: PaymentsService,
              private _messageService: ToastService,
              private store: Store<AppState>,
              private confirmationService: ConfirmationService,
              private messageService: MessageService,

  )
  {
    this.showEdit = false;
    this.Payment = {};
    this.showLoader[0].value = false;
    this.secretKey = '';
  }


  ngOnInit(): void {
    this.getPayments();
    this.store.select('env').subscribe(
      (res) => {
        this.secretKey = res.env;
      },
    );
  }

  public getPayments(): void {
    this.styleTable.dataSource = [];
    this.showLoader[0].value = true;
    this._subscription.add(
      this.PaymentService.getStudentsAll().subscribe((res: any) => {
        if (!res.error) {
          this.Payments = res.data;
          this.Payments.forEach((Payment: any) => {
            const newPayment = {
              value: Payment,
              value1: Payment.name,
              value2: Payment.description,
              value3: Payment.motivo,
              value4: Payment.status === 0 ? 'Deuda' : 'Pagado',
              value5: Payment.date_payment,
              value6: Payment.user_matricula,
              value7: Payment.amount,
              value8: Payment.role,
            }
            this.styleTable.dataSource?.push(newPayment);
          })
          this.getCreateAtMax();
          this.showLoader[0].value = false;
        } else {
          this.showLoader[0].value = false;
          this._messageService.add({
            type: 'error',
            message: 'Inicio de sesión fallido',
            life: 5000,
          });
        }
      })
    )
  }

  public getRoles(roles: Roles[]): string {
    let textTemp: string = '';
    if (roles.length) {
      roles.forEach((data: Roles, index:number)=>{
        if (data.name) {
          textTemp = textTemp + data.name + (index+1 !== roles.length ?', ':'');
        }
      })
    }
    return textTemp;
  }

  public eventTableOption(resp:any): void {
    if (resp.type === 'edit') {
      this.showEdit = true;
      this.Payment = resp.value;
      this.changeStatusContent();
    }
    if (resp.type === 'delete') {
      this.deletePayment(resp.value);
    }
    if (resp.type === 'lock') {
      this.confirmLockUnlock(resp.value);
    }
    if (resp.type === 'update-password') {
      this.showPassword = true;
      this.Payment = resp.value;
    }
  }

  public changeStatusContent(): void {
    this.contentModel.forEach((item: any) => {
      item.status = !item.status;
    })
  }

  public showEventButtonTable(event: any): void {
    this.showEdit = false;
    this.changeStatusContent();
  }

  private deletePayment(Payment: Payment): void {
    this.PaymentSelected = Payment;
    this.showConfirmDelete = true;
  }

  public confirmLockUnlock(Payment: Payment) {
    if (Payment.status === 16) {
      this.confirmationService.confirm({
        message: '¿Desea desbloquear el usuario?',
        header: 'Confirmación de desbloqueo',
        icon: 'pi pi-info-circle',
        accept: () => {
          this.lockUnlock(Payment);
        },
        reject: () => {
          alert('Info')
         this.msgs = [{ severity: 'info', summary: 'Rejected', detail: 'You have rejected' }];
        },
      });
    } else if (Payment.status !== 16) {
      this.confirmationService.confirm({
        message: '¿Desea bloquear el usuario?',
        header: 'Confirmación de bloqueo',
        icon: 'pi pi-info-circle',
        accept: () => {
          this.lockUnlock(Payment);
        },
        reject: () => {
         this.msgs = [{ severity: 'info', summary: 'Rejected', detail: 'You have rejected' }];
        },
      });
    }
  }

  public lockUnlock(Payment: Payment) {
    if (Payment.status !== 16) {
      this.PaymentService.blockPayment('').subscribe((res: Response) => {
        if (res.error) {
          this._messageService.add( {
              type: 'error',
              message: res.msg,
              life: 5000,
            }
          );
        } else {
          this._messageService.add( {
              type: 'success',
              message: res.msg,
              life: 5000,
            }
          );
          Payment.status = 16;
          this.getPayments();
        }
      });
    } else {
      this.PaymentService.unblockPayment('').subscribe((res: Response) => {
        if (res.error) {
          this._messageService.add( {
              type: 'error',
              message: res.msg,
              life: 5000,
            }
          );
        } else {
          this._messageService.add( {
              type: 'success',
              message: res.msg,
              life: 5000,
            }
          );
          Payment.status = 1;
          this.getPayments();
        }
      });
    }
  }

  public onFileExcel(event: any): void {
    const target: DataTransfer = <DataTransfer>(event.target);
    if (target.files.length !== 1) throw new Error('Cannot use multiple files');
    const reader: FileReader = new FileReader();
    reader.onload = (e: any) => {
      const bstr: string = e.target.result;
      const wb: XLSX.WorkBook = XLSX.read(bstr, { type: 'binary' });
      const wsname: string = wb.SheetNames[0];
      const ws: XLSX.WorkSheet = wb.Sheets[wsname];
      this.data = <AOA>(XLSX.utils.sheet_to_json(ws, { header: 1 }));
      this.createPaymentOfFile(this.data);
    };
    reader.readAsBinaryString(target.files[0]);
  }

  public async createPaymentOfFile(data: any[]) {
    console.log(data)
    debugger
    this.showLoader[0].value = true;
    const ArrayPayment: Payment[] = []
    const ArrayRoles: any[] = [];
    if (data.length) {
      data.forEach((us: any, index: number) => {
        debugger
        if (index !== 0 && us.length) {
          debugger
          const newPayment: Payment = {
            dni: us[0],
            Paymentname: us[1],
            names: us[2],
            lastnames: us[3],
            email: us[4],
            sexo: us[5],
            date_birth:  us[6],
            date_admission: us[7],
          }
          debugger
         ArrayRoles.push({id_Payment: us[4], roles: us[7]?.split(',')})
          ArrayPayment.push(newPayment);
        }
      })
      await Promise.all(ArrayPayment).then((Payments: Payment[]) => {
        Payments.forEach((Payment: Payment) => {
          this._subscription.add(
            this.PaymentService.createPayment(Payment).subscribe((resp: any) => {
              if (resp.error) {
                this.showLoader[0].value = false;
                this._messageService.add({
                  type: 'error',
                  message: resp.msg,
                  life: 5000,
                });
                this.dataExport.push(Payment)
              } else {
                this.showLoader[0].value = false;
                this._messageService.add({
                  type: 'success',
                  message: resp.msg,
                  life: 5000,
                });
              }
            })
          );

        })
        this.getPayments();
      })
    }
  }

  public showToast(data: any): void {
    this._messageService.add(data);
  }

  public getCreateAtMax() {
    // @ts-ignore
    this.dateMax = new Date(Math.max(...this.Payments.map((Payment: Payment) => new Date(Payment.created_at))));
  }

  export(): void {
    const ArrayObject:any = [];
    ArrayObject.push(['Nombre de Usuario','N° Identifiación', 'Apellidos Y nombres', 'Correo', 'Estado', 'Roles'])
    this.dataExport.filter((Payment: any) => {
      const ArrayTem = Object.values(Payment)
      ArrayTem.shift();
      ArrayObject.push(ArrayTem);
    })
    // @ts-ignore
    const ws: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet(ArrayObject);
    const wb: XLSX.WorkBook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Roles Exportados');

    XLSX.writeFile(wb, this.fileName);
    this.dataExport = [];
  }

  confirmDialogDelete(event: boolean) {
    this.showConfirmDelete = false;
    const Payment = this.PaymentSelected;
    if (event) {
      this.showLoader[0].value = true;
      this.PaymentService.deletePayment(Payment).subscribe((res: any) => {
        if (res.error) {
          this.showLoader[0].value = false;
          this._messageService.add( {
              type: 'error',
              message: res.msg,
              life: 5000,
            }
          );
        } else {
          this._messageService.add({
            type: 'success',
            message: res.msg,
            life: 5000,
          });

          this.getPayments()
          this.showLoader[0].value = false;
        }
      });
    }else{
      this.PaymentSelected = {};
    }
  }
}
