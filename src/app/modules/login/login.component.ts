...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  private async isLogged() {
    if (this._authenticationService.isLogged()) {
      const modules: Module[] = await this.getModules();
      ...
      if (modules.length > 0) {
        ...
        this.messageService.add(
          {
            type: 'warning',
            message: 'Su rol no tiene módulos asignados o no se pudieron traer los modulos asigandos, se ha cerrado sesión automaticamente!',
            life: 5000
          }
        );
        this.isBlock = false;
  ...
  public getModules(): Promise<any> {
    return new Promise((res, rej) => {
      this.subscription.add(
        this.modulesService.getModulesByRole(
            [
              '70076652-32C0-4E2B-B260-321DBC4A7D93',
              'C1FA60BA-A9AF-45F9-9A25-7B8B224D35BC',
              '33910374-02AC-4B92-AEB1-8595944BFCE8',
              '8B64F8C5-4BC6-4DA1-9B4E-E51B06AEE65F',
              '2982FA50-BB3E-4CBF-9CF1-EA6B8623E937',
            ],
            2,
          ).subscribe(
            (resp) => {
