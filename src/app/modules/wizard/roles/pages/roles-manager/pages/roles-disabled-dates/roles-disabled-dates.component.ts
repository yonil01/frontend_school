...
export class RolesDisabledDatesComponent implements OnInit {
  ...
  fechas: FechasDisall[] = [];
  OpcFechas: FechasDisall = this.fechas[0];
  indexDate = 0;
        ...
        this._roleService.createRolesDateDisallowed(dataDateD).subscribe((res: Response) => {
          if (res.error) {
            ...
            this.fechas.push(<FechasDisall>{
              ...
              idDate: this.DocumentsFechasForm.value.idDate,
              idRol: this.DocumentsFechasForm.value.idRol,
            });
          }
          this.isBlockPage = false;
        });
      } else {
        this._messageService.add({type: 'error', message: 'La Fecha Inicial debe ser Menor a la Fecha Final', life: 5000});
      }
    }
  }

  OpcionTableSet(event: Event, OpcFechas: DateDisallowed, index: number) {
    event.preventDefault();
    this.indexDate = index;
    this.isEdit = true;
    this.DocumentsFechasForm.patchValue({
      ends_at: this.datepipeFormat.transform(OpcFechas.ends_at, 'yyyy-MM-dd'),
      begins_at: this.datepipeFormat.transform(OpcFechas.begins_at, 'yyyy-MM-dd'),
  ...
  confirmDialogDelete(event: boolean){
    if(event){
      const idFecha = this.OpcFechas.idDate.toLocaleLowerCase();
  ...
  updateDateDisallowed(event: Event) {
    ...
    if (FechaI > FechaF) {
      ...
      if (this.indexDate !== 9999) {
        const id_Date = this.fechas[this.indexDate].idDate;
        this.DocumentsFechasForm.value.begins_at = this.DocumentsFechasForm.value.begins_at + 'T00:00:01Z';
        this.DocumentsFechasForm.value.ends_at = this.DocumentsFechasForm.value.ends_at + 'T00:00:01Z';
        const dateDisall: DateDisallowed = {
          ...this.DocumentsFechasForm.value,
        ...
        this._roleService.updateRolesDateDisallowed(dataDateD).subscribe((res: Response) => {
          if (res.error) {
            ...
            this.fechas[this.indexDate].idDate = this.DocumentsFechasForm.value.idDate;
            this.fechas[this.indexDate].idRol = this.DocumentsFechasForm.value.idRol;
